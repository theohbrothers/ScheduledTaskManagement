...
$tasks = Get-ChildItem "C:\path\to\definition\directory\" -File | ? { $_.Extension -eq '.ps1' } | % { . $_ }                                # From multiple .ps1 definition files
$tasks = Get-ChildItem "C:\path\to\definition\directory\" -File | ? { $_.Extension -eq '.json' } | % { Get-Content $_ | ConvertFrom-Json }  # From multiple .json definition files
## Via parameter
